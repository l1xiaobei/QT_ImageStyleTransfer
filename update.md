...
+ 修改了使用imread读取图片这一方式为imdecode，以解决读取中文路径下图片程序会崩溃的问题。

## 0.0.8 (2024-8-13)

### 功能

+ 创建了icon文件夹，用于设置可执行文件的图标
+ 通过pip添加了pyinstaller包，用于打包文件为可执行程序。 
+ 通过执行`pyinstaller -F -w -i "./icon/icon.ico main.py`将文件打包为可执行程序。【由于打包后可执行文件过大，遂放弃。】
